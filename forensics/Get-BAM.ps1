...
$regKeyNames = Get-ChildItem -Name
$data = @()
foreach ($SID in $regKeyNames){
	$keyPath = $regPath + $SID
    $keyProperty = Get-ItemProperty -Path $keyPath
    $Entries = $keyProperty | get-member -MemberType NoteProperty | ? {$_.Name -notmatch "\bPS\w+\b" -and $_.Name -ne "Version" -and $_.Name -ne "SequenceNumber" } | select -ExpandProperty name
